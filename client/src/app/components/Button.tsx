...
const Button: FC<PropsWithChildren<ButtonProps>> = ({
  ...
  const classes: string[] = [
    "flex justify-center items-center font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5",
  ];
  switch (variant) {
    case "outlined":
      // Text Color
    ...
    case "contained":
      ...
      // Background
      if (disabled) {
        classes.push("bg-blue-400 dark:bg-blue-500");
      } else {
  ...
  const renderLoader = () => {
    ...
    const svgClasses = ["inline w-4 h-4 me-3 animate-spin"];
    if (variant === "contained") svgClasses.push("text-white");
    if (variant === "outlined")
      svgClasses.push("text-gray-200 dark:text-gray-600");
    return (
      <svg
        aria-hidden="true"
        role="status"
